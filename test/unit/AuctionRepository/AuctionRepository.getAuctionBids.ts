...
export function getAuctionBids(): void {
  ...
  it("returns an empty Array if the auction does not have bids", async function () {
    const bids = (await this.auctionRepository.getAuctionBids(
      auction2.id,
    )) as Bid[];
  ...
  it("returns an Array with the auction bids", async function () {
    const bids = (await this.auctionRepository.getAuctionBids(
      auction1.id,
    )) as Bid[];

    expect(bids.length).to.equal(1);
    expect(bids[0].from).to.equal(bidderAddr);
